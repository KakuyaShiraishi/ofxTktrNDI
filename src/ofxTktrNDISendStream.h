...
template<typename FrameType, typename Type=ofxTktrNDISender>
class Stream
{
public:
	...
	void send(Src &&src, const std::string &metadata="", int64_t timecode=NDIlib_send_timecode_synthesize) {
		static_assert(!std::is_same<Frame, MetadataFrame>::value, "this function is not for ofxTktrNDISendMetadata");
...
class MetadataStream : public Stream<ofxTktrNDI::MetadataFrame>
{
public:
...
using ofxTktrNDISendVideo = ofxTktrNDI::Send::VideoStream;
using ofxTktrNDISendAudio = ofxTktrNDI::Send::Stream<ofxTktrNDI::AudioFrame>;
using ofxTktrNDISendMetadata = ofxTktrNDI::Send::MetadataStream;
using ofxTktrNDISendMetadataRecv = ofxTktrNDI::Recv::Stream<ofxTktrNDI::MetadataFrame, ofxTktrNDISender>;
